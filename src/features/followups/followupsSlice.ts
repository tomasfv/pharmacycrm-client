  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        err.response?.data?.message || "Failed to fetch follow-ups",
      );
    }
  },
);
  ...
  async (
    {
      followUpId,
      newStatus,
    }: { followUpId: string; newStatus: FollowUpStatus },
    { dispatch, rejectWithValue },
  ) => {
    try {
      const { data } = await followupsApi.update(followUpId, {
        status: newStatus,
      } as any);
...
export const addFollowUp = createAsyncThunk(
  "followups/add",
  async (
    followUpData: Omit<FollowUp, "id" | "createdAt" | "updatedAt">,
    { rejectWithValue },
  ) => {
    try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(moveFollowUp.fulfilled, (state, action) => {
        const idx = state.followUps.findIndex(
          (f) => f.id === action.payload.id,
        );
        if (idx >= 0) state.followUps[idx] = action.payload;
      })
      .addCase(moveFollowUp.rejected, (state) => {
        state.loading = false;
      })
...
export const selectFollowUpsByStatus =
  (status: FollowUpStatus) => (state: { followups: FollowUpsState }) =>
